...
import { useRouter, useSearchParams } from "next/navigation";
import { useAdminAuth, RateLimitError } from "@/lib/hooks/useAdminAuth"; // ← updated import
                ...
                <div className="bg-white rounded-3xl shadow-2xl p-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <button type="submit" disabled={disabled}
                            className="w-full py-4 rounded-xl font-bold text-white text-sm tracking-wide transition-all disabled:opacity-60 disabled:cursor-not-allowed active:scale-[0.99]"
                            style={{ background: disabled ? "#6B7280" : "linear-gradient(135deg, #0D1B2A 0%, #1A5C4A 100%)" }}>
                            {loading ? (
                                <span className="flex items-center justify-center gap-2">
                                    <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                                    Signing in…
