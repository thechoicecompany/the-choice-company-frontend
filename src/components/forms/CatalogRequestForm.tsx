...
export default function CatalogRequestForm() {
    ...
    async function onSubmit(data: InquiryFormData) {
        ...
        try {
            ...
            // Unlock the gate so gallery downloads work going forward.
            markCatalogUnlocked();
            ...
            // Send them back to wherever they came from (default: /gallery).
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Field label="Mobile *" error={errors.mobile?.message}>
                        <input type="tel" maxLength={10} placeholder="10-digit mobile number" {...register("mobile")} className="input" />
                    </Field>
                    ...
                    <Field label="Quantity Required (min 50) *" error={errors.quantityRequired?.message}>
                        <input type="number" min={50} max={500000} {...register("quantityRequired", { valueAsNumber: true })} className="input" />
                    </Field>
                ...
                <Field label="Packaging Requirements">
                    <input placeholder="e.g. Gift-wrapped, branded boxes…" {...register("packagingRequirement")} className="input" />
                </Field>

                <Field label="What are you looking for? *" error={errors.additionalNotes?.message}>
                    <textarea rows={4} placeholder="e.g. Employee kits for 500 staff, festive hampers under ₹800/unit, need samples before bulk order…" {...register("additionalNotes")} className="input resize-none" />
                </Field>
