...
export default function ProductFilters({ activeFilters }: Props) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-1">
          {CATEGORIES.map((cat) => (
            <Link key={cat.slug} href={buildHref("category", cat.slug)}
              ...
              <span className={`w-2 h-2 rounded-full flex-shrink-0 ${activeFilters.category === cat.slug ? "bg-gold" : "bg-gray-300"}`} />
              {cat.label}
            </Link>
          ))}
        </div>
      ...
      {/* Budget */}
      <div>
        <h4 className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-3">Budget per Kit</h4>
        <div className="space-y-1">
          {BUDGET_RANGES.map((b) => (
            <Link key={b.value} href={buildHref("budget", b.value)}
              className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${
                activeFilters.budget === b.value
      ...
      {/* Occasion */}
      <div>
        <h4 className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-3">Occasion</h4>
      ...
      {/* MOQ */}
      <div>
        <h4 className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-3">Min. Order Qty</h4>
        {["50","100","250","500"].map((moq) => (
          <Link key={moq} href={buildHref("moq", moq)}
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${
              activeFilters["moq" as keyof typeof activeFilters] === moq
                ? "bg-navy text-white font-medium"
                : "text-gray-600 hover:bg-gray-50"
            }`}>
