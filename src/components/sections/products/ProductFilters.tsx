      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1">
          {OCCASIONS.slice(0, 8).map((occ) => (
            <Link
              key={occ.slug}
              href={buildHref("occasion", occ.slug)}
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1">
          {["50", "100", "250", "500"].map((moq) => (
            <Link
              key={moq}
              href={buildHref("moq", moq)}
              className={`px-2 py-0.5 rounded-full text-[10px] border transition-colors ${activeFilters.moq === moq
