...
//     // ── Categories: multi-select. Falls back to legacy single `category`
//     // for products created before this change. ──────────────────────────
...
//         // ── FIX: only send newly-uploaded images to the backend.
//         // Slots that came from the DB already have dbImageId — re-sending
//         // them would create duplicate rows on every edit save.
//         // This mirrors the pattern SampleProductForm already uses correctly.
...
//                 publicId: s.publicId ?? "",
//                 sortOrder: i,
//                 isPrimary: i === 0 && !initial, // only mark primary on create
//             }));

//         // Still derive image / images from the full finalSlots list so the
//         // legacy scalar fields on the payload always reflect the current state.
//         const allImages: UploadedImageRef[] = finalSlots.map((s, i) => ({
//             url: s.url!,
//             publicId: s.publicId ?? "",
//             sortOrder: i,
//             isPrimary: i === 0,
...
//         // ── Derive category slugs from the selected labels. First category
//         // is sent as the legacy singular `category`/`categorySlug` too, for
//         // backward-compat with anything still reading the single field. ────
//         const categorySlugs = categories.map(
//             label => CATEGORIES.find(c => c.label === label)?.slug ?? sanitizeSlug(label)
//         );
...
//                 images: allImages.slice(1).map(i => i.url),
//                 moq,
//                 basePrice: price,
...
//                     },
//                 }),
//                 // Only newly-uploaded images — existing DB images are excluded
//                 // to prevent duplicate row inserts on every edit.
...
//                                     className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all disabled:opacity-50 ${categories.includes(c.label)
...
//                                 : `${categories.length} selected — first one (${categories[0]}) is used as the primary category`}
//                         </p>
//                     </div>
